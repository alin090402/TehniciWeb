...
CREATE TYPE categ_coperta AS ENUM( 'cartonata', 'lucioasa', 'simpla', 'piele');
CREATE TYPE tipuri_produse AS ENUM('fictiune', 'nonfictiune', 'copii', 'nespecificat');
...
CREATE TABLE IF NOT EXISTS prajituri (
   ...
   tip_produs tipuri_produse DEFAULT 'nespecificat',
   varsta_minima INT NOT NULL CHECK (varsta_minima>=0),
   categorie categ_prajitura DEFAULT 'comuna',
   autori VARCHAR [], --pot sa nu fie specificare deci nu punem NOT NULL
   varianta_digitala BOOLEAN NOT NULL DEFAULT FALSE,
   imagine VARCHAR(300),
